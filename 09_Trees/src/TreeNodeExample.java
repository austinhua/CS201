...
public class TreeNodeExample<Type extends Comparable<Type>> {
    ...
    private boolean containsNode(Type value, TreeNode current) {
        // base case: if the node is null, it does not contain the value
    	if (current == null) return false;
    	if (current.myValue.equals(value)) return true;
    	
    	if (containsNode(value, current.myLeft)) return true;
    	if (containsNode(value, current.myRight)) return true;
    	return false;
    }
    ...
    private int findMax(TreeNode current) {
    	...
    	return Math.max(Math.max((int)current.myValue, findMax(current.myLeft)), Math.max((int)current.myValue, findMax(current.myRight)));
    }
